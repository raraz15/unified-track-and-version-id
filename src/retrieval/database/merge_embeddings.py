...
def collect_all_shapes(emb_paths: list[Path], num_workers: int = 8):
...
def merge_embeddings_to_memmap(
    embeddings_dir: Path,
    flush_frequency: int = 5000,  # How often to flush the memmap to disk
    num_workers: int = 6,
) -> tuple[Path, Path]:
    ...
    # We store the merged embeddings in a memmap file in the embeddings directory
    db_mm_path = embeddings_dir / "database.mm"
    db_csv_path = embeddings_dir / "database.csv"
    ...
    if db_mm_path.exists():
        ...
        if user_input == "y":
            ...
            if (
                input("Do you want to use the existing database? (y/n): ")
                .strip()
                .lower()
                == "n"
            ):
    ...
    print(f"Getting the embedding shapes with {num_workers} threads...")
    fnames_and_lens, emb_dim = collect_all_shapes(emb_paths, num_workers=num_workers)
    ...
    print(f"Creating the merged database embeddings in {str(db_mm_path)}")
    ...
    memmap = np.memmap(db_mm_path, dtype=np.float16, mode="w+", shape=db_shape)
    memmap.flush()

    print(f"Merging with {num_workers} threads…")
    futures = []
    with ThreadPoolExecutor(max_workers=num_workers) as exe:
    ...
    with open(db_csv_path, "w", newline="") as csvfile:
        ...
        writer.writerow(["emb_file_name", "start", "end", "emb_dim"])
        for fname, (start, end) in zip(fnames_and_lens.keys(), starts_ends):
            writer.writerow([fname, start, end, emb_dim])
    print(f"Database CSV created at {db_csv_path}")

    print(f"Total elapsed time: {sec_to_hms(t0, True)[1]}")
