...
class AddBackgroundNoise(BaseWaveformTransform):
    ...
    * https://github.com/microsoft/DNS-Challenge/
    ...
    # NOTE OGUZ: to bypass checks, we will force mono inside the code
    ...
    def __init__(
        ...
        self.sound_file_paths = list(Path(self.sounds_path).rglob("*.wav"))
    ...
    def apply(
        ...
        noise_sound = self._load_sound(self.parameters["noise_file_path"], sample_rate)

        # Repeat the sound if it is shorter than the input sound
        ...
        if noise_sound.shape[1] > num_samples:
            noise_sound = noise_sound[:, :num_samples]
